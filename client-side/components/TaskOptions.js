...
export default function Task ({token,verify_token,item,setItem,setShowItem,makeTodos,event}){
 ...
 return <View style={item.status==="booked" ?styles.singleBooked :item.status==="done" ?styles.singleDone :item.status==="suggested" ?styles.singleSug :styles.single}>
            <View style={styles.singleTop}>
                <Text style={styles.singleName}>{item.task}</Text>
            </View> 
            ...
            <View style={styles.listBtns}>           
               <TouchableOpacity onPress={()=>{setShowItem(false)}} style={styles.back}>
                  <Text style={styles.btnsText}>BACK</Text>
               </TouchableOpacity> 
            {!item.booked && !item.done &&
               <TouchableOpacity onPress={()=>{verify_token(),mark("booked")}} style={styles.book}>
                  <Text style={styles.bookText}>BOOK</Text>
               </TouchableOpacity> 
            }
            {!item.done &&
               <TouchableOpacity onPress={()=>{verify_token(),mark("done")}} style={styles.done}>
